<template>
  <NuxtLayout
    ...
    :breadcrumbs="[
      ...
      { title: 'Community Dialogue', href: '/activities/community-dialogue', current: true },
    ]"
  >
    <div class="cd-page">
      <div class="page-header">
        <h1 class="page-title">Community Dialogue</h1>
        <p class="page-subtitle">Track 7 theme-based community sessions.</p>
      </div>
      ...
      <div v-else-if="!selectedGroup" class="empty-state">
        Select or create a community dialogue group to begin.
      </div>
      ...
      <template v-else>
        ...
        <div class="session-controls">
          <div class="field">
            ...
            <select v-model="currentTheme" class="field-input">
              <option v-for="(t, i) in themes" :key="i" :value="i">{{ t.name }}</option>
            </select>
          </div>
          <div class="field">
        ...
        <div class="submit-bar">
          <button class="btn-primary" :disabled="submitting" @click="submitSessionAttendance">
      ...
      <div v-if="showCreate" class="modal-overlay" @click.self="showCreate = false">
        <div class="modal-box">
          <div class="modal-header">
            <h3 class="modal-title">Create Community Dialogue Group</h3>
            <button class="modal-close" @click="showCreate = false">&times;</button>
          </div>
          <div class="modal-body">
            <div class="field">
              <label class="field-label">Group Name</label>
              <input v-model="newGroup.name" type="text" class="field-input" placeholder="e.g. CD Group — Zone A" />
            </div>
            <div class="field">
...
import { useCommunityDialogue, CD_THEMES } from '../../composables/useCommunityDialogue'
...
} = useCommunityDialogue()

const selectedGroupId = ref('')
const showCreate = ref(false)
const showEnroll = ref(false)
const themes = CD_THEMES

const newGroup = reactive({ name: '', service_point_id: '' })
...
async function handleCreate() {
  const activityId = authStore.frameworkActivities?.find(
    (a: any) => a.code === 'community_dialogue' || a.template_code === 'community_dialogue',
  )?.id || ''
...
const gridColumns = computed<GridColumn[]>(() =>
  themes.map(t => ({ label: t.name.split(' ')[0], moduleName: t.name })),
)

const activeColumn = computed(() => currentTheme.value)
...
const gridRows = computed<GridRow[]>(() =>
  enrollments.value
    ...
    .map(e => ({
      ...
      attended: gridColumns.value.map(() => false),
